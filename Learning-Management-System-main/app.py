...
# Set the expiration period in days
EXPIRATION_DAYS = 10

# File to store the first run timestamp
TIMESTAMP_FILE = 'app_timestamp.dat'
...
def check_expiration():
    ...
    with open(TIMESTAMP_FILE, 'r') as f:
        try:
            ...
            # If the timestamp is invalid, create a new one
    ...
    elapsed_days = (current_time - timestamp) / (60 * 60 * 24)
    
    # Check if the application has expired
    return elapsed_days > EXPIRATION_DAYS
...
# Add a route for expired application
@app.route('/expired')
def expired():
        ...
        with open(TIMESTAMP_FILE, 'r') as f:
            try:
                ...
                expiration_date = datetime.datetime.fromtimestamp(
                    timestamp + EXPIRATION_DAYS * 24 * 60 * 60
                ).strftime('%Y-%m-%d')
            except ValueError:
    ...
    return render_template('expired.html', expiration_date=expiration_date, now=now)

# Modify the existing routes to check for expiration
@app.route('/')
def index():
    if check_expiration():
...
if __name__ == '__main__':
    ...
    # Check if the application is about to expire
    if os.path.exists(TIMESTAMP_FILE):
        with open(TIMESTAMP_FILE, 'r') as f:
            try:
                timestamp = int(f.read().strip())
                elapsed_days = (int(time.time()) - timestamp) / (60 * 60 * 24)
                remaining_days = EXPIRATION_DAYS - elapsed_days
                
                if remaining_days > 0:
                    print(f"Application will expire in {remaining_days:.1f} days")
                    if remaining_days <= 3:
                        print("WARNING: Application expiration is imminent!")
                else:
                    print("Application has expired! Redirecting to expiration page.")
            except ValueError:
                print("Invalid timestamp found. Creating a new one.")
                with open(TIMESTAMP_FILE, 'w') as f:
                    f.write(str(int(time.time())))
